...
- 🎯 Entry points to execute your program using `python -m <bs>` or `$ bs` with basic CLI argument parsing.
...
---
# bs Flask Application

Awesome bs created by Airhard
...
```bash
git clone https://github.com/Airhard/BS bs
cd bs
make install
```
...
```bash
bs create-db   # run once
bs populate-db  # run once (optional)
bs add-user -u admin -p 1234  # ads a user
bs run
```
